...
*Set baseMVA to be 100MVA and line thermal limits at 5pu (assume 5000MVA limit)
Scalar BaseMVA / 100 /;
...
1          -9       9      -9       9       1       1             0
2           1       1    -2.5     1.5    0.98    1.02
3       -1.75   -1.75   -0.65   -0.65     0.8     1.2
4        1.20    1.20      -3     1.8    0.98    1.02
5       -1.45   -1.45   -0.55   -0.55     0.8     1.2
;

* Size of capacitors added to loads in p.u. (used in Case studies 3 and 4)
...
* Amount of load curtailed from each bus (used in Case study 4)
...
P.up(i)=Limits(i,'Pmax');
P.lo(i)=Limits(i,'Pmin');
...
DUMMY ..                         D =e= 1;
Pflow(i) ..                      P(i) =e= SUM(j,V(i)*V(j)*YBusMag(i,j)*cos(YBusAngle(i,j)+delta(j)-delta(i)))-loadCurt(i,'LC');
Qflow(i) ..                      Q(i) =e= SUM(j,-V(i)*V(j)*YBusMag(i,j)*sin(YBusAngle(i,j)+delta(j)-delta(i)));
PLineFct(i,j) ..                 PLine(i,j)=e=V(i)*V(i)*g0(i,j)-V(i)*V(j)*(g0(i,j)*cos(delta(j)-delta(i))-b0(i,j)*sin(delta(j)-delta(i)));
QLineFct(i,j) ..                 QLine(i,j)=e=-V(i)*V(i)*(b0(i,j)+(1/2)*yCharging(i,j))+V(i)*V(j)*(g0(i,j)*sin(delta(j)-delta(i))+b0(i,j)*cos(delta(j)-delta(i)));
